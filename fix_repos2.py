...
FILTER = """
if [ "$GIT_AUTHOR_EMAIL" = "shubham@users.noreply.github.com" ] || [ "$GIT_AUTHOR_EMAIL" = "shubhamkumar997800@gmail.com" ]; then
    export GIT_AUTHOR_NAME="Shubham Dangi"
    export GIT_AUTHOR_EMAIL="shubhamkumars997800@gmail.com"
fi
if [ "$GIT_COMMITTER_EMAIL" = "shubham@users.noreply.github.com" ] || [ "$GIT_COMMITTER_EMAIL" = "shubhamkumar997800@gmail.com" ]; then
...
for repo in repos:
    print(f"=== Fixing {repo} ===")
    tmp = os.path.join(tempfile.gettempdir(), f"pyfix2_{repo}")
    ...
    subprocess.run(["git", "filter-branch", "-f", "--env-filter", FILTER, "--", "--all"], 
